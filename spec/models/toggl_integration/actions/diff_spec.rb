...
describe TogglIntegration::Actions::Diff do
  describe '#call' do
    ...
    let(:server_teams) do
      [
        TogglIntegration::Team.new('team1', ['john.doe'], ['team1']),
        TogglIntegration::Team.new('team2', ['john.doe', 'james.bond', 'john.wayne'], ['team2']),
      ]
    end
    ...
    it 'returns list of members to add' do
      ...
      expect(diff_result[:add_members]['team1']).to eq ['james.bond']
    end

    it 'returns list of members to remove' do
      expect(diff_result[:remove_members].size).to eq 1
      expect(diff_result[:remove_members]['team2']).to eq ['john.wayne']
    end

    it 'returns list of teams to create' do
      expect(diff_result[:create_teams].size).to eq 2
      expect(diff_result[:create_teams]['team3']).to eq [team3]
      expect(diff_result[:create_teams]['team4']).to eq [team4]
    end
  end
end
