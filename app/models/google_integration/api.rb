module GoogleIntegration
  class Api
    ...
    def list_groups
      data = get "groups", domain: AppConfig.email_domain
      data['groups']
    end

    def create_group(name)
      response = post "groups", email: name, name: "Project group - #{name}",
        description: "Project group for #{name} - includes pm group."
    ...
    def post_filters(email)
      ...
      Dir.glob("#{Rails.root}/static_data/gmail_filters/*").each do |filter|
        ...
        api.post "https://apps-apis.google.com/a/feeds/emailsettings/2.0/netguru.pl/#{username}/filter",
        body: filter,
    ...
    def client
      @client ||= OAuth2::Client.new(AppConfig.google.client_id, AppConfig.google.client_secret, {
